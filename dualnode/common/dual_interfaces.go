...
func UnpackDualEventIntoMap(a *abi.ABI, dualEvent *dualTypes.DualEvent, src int64) (interface{}, int64, int64, error) {
	...
	data, err := hex.DecodeString(strings.TrimPrefix(dualEvent.Raw.Data.String(), "0x"))
	if err != nil {
		return nil, -1, -1, err
	}
	...
	topics := make([]common.Hash, len(dualEvent.Raw.Topics)-1)
	for i, topic := range dualEvent.Raw.Topics[1:] { // exclude the eventID (dualEvent.Topic[0])
		topics[i] = topic
	}
