...
// It is part of the filter package since polling goes with kai_getFilterChanges.
//
// https://github.com/kardiachain/go-kardia/wiki/Kardia-JSON-RPC-API#kai_newBlockFilter
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(kardia.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as kardia.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria kardia.FilterQuery
...
// Using "latest" as block height will return logs for mined blocks.
// Using "pending" as block height returns logs for not yet mined (pending) blocks.
// In case logs are removed (chain reorg) previously returned logs are returned
// again but with the removed property set to true.
//
// In case "fromBlock" > "toBlock" an error is returned.
//
// https://github.com/kardiachain/go-kardia/wiki/Kardia-JSON-RPC-API#kai_newFilter
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	logs := make(chan []*types.Log)
	logsSub, err := api.events.SubscribeLogs(kardia.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
...
//
// https://github.com/kardiachain/go-kardia/wiki/Kardia-JSON-RPC-API#kai_uninstallFilter
...
//
// https://github.com/kardiachain/go-kardia/wiki/Kardia-JSON-RPC-API#kai_getFilterChanges
...
func (args *FilterCriteria) UnmarshalJSON(data []byte) error {
	type input struct {
		...
		FromBlock *rpc.BlockHeight `json:"fromBlock"`
		ToBlock   *rpc.BlockHeight `json:"toBlock"`
	...
	if raw.BlockHash != nil {
		...
		if raw.FromBlock != nil {
			args.FromBlock = raw.FromBlock.Uint64()
		}

		if raw.ToBlock != nil {
			args.ToBlock = raw.ToBlock.Uint64()
		}
	}
